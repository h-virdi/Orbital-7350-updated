...
Object.values(grouped).forEach(eventsArray => {
  ...
  .map(([date, data]) => {
    const sortedData = [...data].sort((a, b) => {
      // Convert time like "2:30pm" into minutes for comparison
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditing) {
  const eventRef = doc(db, 'users', user.uid, 'events', currentlyEditingEventId!);
...
<Modal visible={!!selectedEvent} animationType="slide" transparent={true}>
  ...
  onPress={async () => {
    try {
      ...
      const docRef = doc(db, 'users', user.uid, 'events', selectedEvent.id);
