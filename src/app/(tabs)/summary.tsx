 ...
 const handlePress = () => {
  ...
  try {
    ...
    if (currentlyEditingTripId) {
      await updateDoc(doc(db, 'trips', currentlyEditingTripId), tripData);
...
<Button
  ...
  onPress={() => {
    ...
    setForm(true); // open form modal
    setEditModalVisible(false); // close view modal
  }}
/>
            ...
            <Button
              ...
              onPress={async () => {
                try {
                  await deleteDoc(doc(db, 'users', auth.currentUser!.uid, 'trips', selectedTrip.id));
